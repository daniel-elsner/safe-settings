...
module.exports = class CustomProperties extends Diffable {
  ...
  async find () {
    ...
    return existing.filter(({ name, value }) => {
  ...
  // Generate a map of custom properties and their default values for the organization
  async getOrgPropertyDefaultMap () {
    try {
      ...
      for (const prop of data) {
        // don't handle multi_select properties in here to reduce complexity
        ...
        defaults[prop.property_name.toLowerCase()] = prop.default_value
      }
